...
class DataPreparationService:
    ...
    async def extract_demand_dataset(session: AsyncSession) -> pd.DataFrame:
        ...
        prod_map = {r.sku: (r.category, r.criticality, r.unit_cost) for r in prod_res.all()}
        ...
        df_demand["unit_cost"] = df_demand["sku"].map(lambda s: prod_map[s][2] if s in prod_map else 10.0)
