...
from backend.app.database import AsyncSessionLocal, init_database, check_db_health
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.error(
            ...
            "Server is in explicit Database-Unavailable state. No fallback data will be served."
        )
    yield
...
async def health_check():
    ...
    is_db_connected = await check_db_health()
    if is_db_connected:
        return {
            "status": "HEALTHY",
    ...
    return JSONResponse(
        ...
        content={
            ...
            "database_engine": "PostgreSQL",
            "error": "PostgreSQL database is unavailable. No fallback data is active."
        }
    )
...
if active_dist_path is not None:
    spa_dir: str = active_dist_path
