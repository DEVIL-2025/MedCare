...
class DemandSensingEngine:
    ...
    @staticmethod
    async def scan_and_record_demand_surges(
        session: AsyncSession,
        horizon_days: int = settings.FORECAST_HORIZON_DAYS
    ) -> List[DemandSurgeEvent]:
        """
        ...
        all_forecasts = await PredictionService.predict_all_demands(session, horizon_days)
        surge_events = []
        for sku, wh_id in sku_dc_pairs:
            f_data = all_forecasts.get(f"{sku}_{wh_id}") or all_forecasts.get(f"{sku}_{wh_id}_{horizon_days}")
            if f_data and f_data.get("surge_detected"):
                surge = DemandSurgeEvent(
                    ...
                    severity="CRITICAL" if f_data["surge_pct"] >= 50.0 else "HIGH",
